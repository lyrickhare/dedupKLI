...
class Classifier:
    docNameCodesDict = {"0":"Aadhaar","1":"other","2":"PAN"}

    docNameCodesLenght = len(docNameCodesDict)
    ...
    def get_prediction(self,image,confidence=1/(docNameCodesLenght),CodesDict=docNameCodesDict):
        ...
        if self.MODEL_LOADED==False:
            ...
            if self.FAULTY_IMAGE==True:
                ...
                self.class_probabilities = self.model.predict(self.x)
                self.class_probabilities=self.class_probabilities[0]
                if max(self.class_probabilities)<=confidence:
                    self.pred_class='other'
                else:
                    self.pred_class = CodesDict[str(self.class_probabilities.argmax())]
